...
async def test_python_m_gowui_serves_prints_its_url_and_saves_on_sigint(tmp_path):
    ...
    try:
        ...
        # §9 "Stopping": after the save the process dies by SIGINT and prints no traceback.
        stderr.flush()
        errors = (tmp_path / "stderr.txt").read_text()
        assert (proc.returncode, "Traceback" in errors) == (-signal.SIGINT, False), errors[-2000:]
    finally:
